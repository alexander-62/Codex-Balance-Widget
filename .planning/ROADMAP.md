...
- [ ] **Phase 3: Shared library extraction + Codex migration** - Extract `usage_widget_common` from Codex's existing, tested patterns and migrate the Codex widget onto it with no behavior change.
- [ ] **Phase 4: Claude widget adoption + bugfixes** - Wire the Claude widget onto the shared library, fixing its tooltip-truncation crash and slow 401 retry as part of the rewiring.
