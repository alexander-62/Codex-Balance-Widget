...
1. **Как выглядит ответ при активном 5h-окне (оба окна сразу)?**
   ...
   - RESOLVED: вопрос эмпирический (зависит от состояния аккаунта, не от кода) и не блокирует цель фазы — закрыт операционным решением, а не дополнительным ресёрчем. Recommendation реализован дословно в `probe_wham_usage.py` (план 01-01, Task 2): скрипт печатает все найденные окна независимо от их количества.
